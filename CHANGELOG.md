...
### 修复
- **签名统一**：本地构建与 GitHub Actions 现在共用仓库里的 `keystore/debug.keystore`。
  之前 AGP 用的是「每台机器各自生成」的 debug keystore，于是 Release 里的 APK 和本地构建的 APK
  签名不一样 —— 覆盖安装直接报「应用未安装」，等于每个版本都得先卸载。
  这把是**公开的调试签名**（口令 `android`），只为让升级能装上；要上架请自己配正式签名。

  ⚠ 从 1.0.1 及更早升到 1.0.2 需要**先卸载旧版一次**；1.0.2 之后就能直接覆盖安装。
